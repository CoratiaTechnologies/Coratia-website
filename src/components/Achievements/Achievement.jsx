  ...
  return (
    ...
    `}</style>
      ...
      <div className="w-full px-5 bg-white">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 w-max mx-auto md:gap-x-8 lg:gap-x-16 gap-y-5 md:gap-y-10">
          {achievementcontent.map((item) => (
            <div
              key={item.id}
              className="card rounded-xl border-2 shadow-xl md:w-[400px] md:h-[280px] w-[300px] h-[240px] overflow-hidden cursor-pointer"
            >
              <div>
                <div
                  className="text-center text-sm md:text-base text-black items-center
                flex hover-text relative md:h-[230px] z-30 w-full"
                >
                  {item.desc}
                </div>
                <Image
                  ...
                  width={540}
                  height={280}
                  className="md:h-[230px] h-[170px] w-full object-cover"
                />
              </div>
